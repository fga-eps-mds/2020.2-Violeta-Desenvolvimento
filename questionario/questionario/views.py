...
def analisa_fluxos(request):
    ...
    for quest in quests:
        ...
        if perg_db['question'] == perg_in['question']:
            ...
            resp_db = perg_db['children'][0]
            if resp_db['label'] == resp_in['label'] and resp_in['label'] == 'Sim':
                # Isso lhe causou algum constrangimento como xingamentos, humilhações e/ou exposições?
                perg_in = resp_in
                perg_db = resp_db
                if perg_db['question'] == perg_in['question']:
                    resp_in = perg_in['children'][0]
                    resp_db = perg_db['children'][0]
                    if resp_db['label'] == resp_in['label'] and resp_in['label'] == 'Sim':
                        # Esse acontecimento foi usado como justificativa para algum tipo de agressão física?
                        perg_in = resp_in
                        perg_db = resp_db
                        if perg_db['question'] == perg_in['question']:
                            resp_in = perg_in['children'][0]
